$.ajax({
  ...
  success: function(data) {
    ...
    const accumulateCrimes = (arr, prop) => arr.reduce(function(accumulator, currentValue) {
    ...
    const generateTotalObject = (arr, gender, ageGroup, background) => {
        ...
        "vermogensmisdrijven": accumulateCrimes(arr.filter(item => item.Migratieachtergrond === background), "vermogensmisdrijven"),
        "vernielingen": accumulateCrimes(arr.filter(item => item.Migratieachtergrond === background), "vernielingen"),
        "geweldsmisdrijven": accumulateCrimes(arr.filter(item => item.Migratieachtergrond === background), "geweldsmisdrijven"),
        "verkeersmisdrijven": accumulateCrimes(arr.filter(item => item.Migratieachtergrond === background), "verkeersmisdrijven"),
        "drugsmisdrijven": accumulateCrimes(arr.filter(item => item.Migratieachtergrond === background), "drugsmisdrijven")
      }
    };

    function getFilterdData(dataObject, age, gender, background) {
      const dataRaw = dataObject.filter(
          item => item.Leeftijd === age && item.Geslacht === gender && item.Migratieachtergrond === background);
      data = dataRaw;
      return data;
    }
    ...
    function getCleanData(data,gender, age, background){
      ...
      const Object =  generateTotalObject(filtererdData, gender, age, background);
      let sum  = (Object.vermogensmisdrijven + Object.vernielingen + Object.geweldsmisdrijven + Object.verkeersmisdrijven + Object.drugsmisdrijven);
      return {
    ...
    for(const property in array){
       ...
       newObject.push(getCleanData(data,array[property].gender,array[property].age,array[property].background));
    }
